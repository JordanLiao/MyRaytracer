...
Camera::Camera(glm::vec3 pos, glm::vec3 look, float aspectRatio, float vfov, unsigned int width, unsigned int height, glm::vec3 up) {
	this->pos = pos;
	this->look = look;
	this->up = up;
...
void Camera::generateRay(unsigned int x, unsigned int y, glm::vec3& ray) {
	...
	//taking the easy way first by not considering up vector, assume cam looks into -z, and stays at world origin
